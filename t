...
class Database(object):
    ...
    def _replay_log(self):
        ...
        with open(fname, 'r') as f:
            log = map(json.loads, filter(lambda x: x != '', f.read().split('\n')))
    ...
    def finished_tasks(self):
        return list(filter(lambda t: t.status == Task.FINISHED,
            self._tasks.values()))

    def pending_tasks(self):
        return list(filter(lambda t: t.status == Task.PENDING,
            self._tasks.values()))

    def cancelled_tasks(self):
        return list(filter(lambda t: t.status == Task.CANCELLED,
            self._tasks.values()))
